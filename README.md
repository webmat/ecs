...
| Field  | Level  | Type  | Description  | Example  |
|---|---|---|---|---|
| <a name="error.id"></a>error.id | core | keyword | Unique identifier for the error. |  |
| <a name="error.message"></a>error.message | core | text | Error message. |  |
| <a name="error.code"></a>error.code | core | keyword | Error code describing the error. |  |
...
| Field  | Level  | Type  | Description  | Example  |
|---|---|---|---|---|
| <a name="event.id"></a>event.id | core | keyword | Unique ID to describe the event. | `8a4f500d` |
| <a name="event.category"></a>event.category | core | keyword | Event category.<br/>This can be a user defined category. | `metrics` |
| <a name="event.type"></a>event.type | core | keyword | A type given to this kind of event which can be used for grouping.<br/>This is normally defined by the user. | `nginx-stats-metrics` |
| <a name="event.action"></a>event.action | core | keyword | The action captured by the event. The type of action will vary from system to system but is likely to include actions by security services, such as blocking or quarantining; as well as more generic actions such as login events, file i/o or proxy forwarding events.<br/>The value is normally defined by the user. | `reject` |
| <a name="event.module"></a>event.module | core | keyword | Name of the module this data is coming from.<br/>This information is coming from the modules used in Beats or Logstash. | `mysql` |
| <a name="event.dataset"></a>event.dataset | core | keyword | Name of the dataset.<br/>The concept of a `dataset` (fileset / metricset) is used in Beats as a subset of modules. It contains the information which is currently stored in metricset.name and metricset.module or fileset.name. | `stats` |
| <a name="event.severity"></a>event.severity | core | long | Severity describes the severity of the event. What the different severity values mean can very different between use cases. It's up to the implementer to make sure severities are consistent across events. | `7` |
| <a name="event.original"></a>event.original | core | keyword | Raw text message of entire event. Used to demonstrate log integrity.<br/>This field is not indexed and doc_values are disabled. It cannot be searched, but it can be retrieved from `_source`. | `Sep 19 08:26:10 host CEF:0&#124;Security&#124; threatmanager&#124;1.0&#124;100&#124; worm successfully stopped&#124;10&#124;src=10.0.0.1 dst=2.1.2.2spt=1232` |
| <a name="event.hash"></a>event.hash | extended | keyword | Hash (perhaps logstash fingerprint) of raw field to be able to demonstrate log integrity. | `123456789012345678901234567890ABCD` |
| <a name="event.version"></a>event.version | core | keyword | The version field contains the version an event for ECS adheres to.<br/>This field should be provided as part of each event to make it possible to detect to which ECS version an event belongs.<br/>event.version is a required field and must exist in all events. It describes which ECS version the event adheres to.<br/>The current version is 0.1.0. | `0.1.0` |
| <a name="event.duration"></a>event.duration | core | long | Duration of the event in nanoseconds. |  |
| <a name="event.created"></a>event.created | core | date | event.created contains the date when the event was created.<br/>This timestamp is distinct from @timestamp in that @timestamp contains the processed timestamp. For logs these two timestamps can be different as the timestamp in the log line and when the event is read for example by Filebeat are not identical. `@timestamp` must contain the timestamp extracted from the log line, event.created when the log line is read. The same could apply to package capturing where @timestamp contains the timestamp extracted from the network package and event.created when the event was created.<br/>In case the two timestamps are identical, @timestamp should be used. |  |
| <a name="event.risk_score"></a>event.risk_score | core | float | Risk score or priority of the event (e.g. security solutions). Use your system's original value here. |  |
| <a name="event.risk_score_norm"></a>event.risk_score_norm | extended | float | Normalized risk score or priority of the event, on a scale of 0 to 100.<br/>This is mainly useful if you use more than one system that assigns risk scores, and you want to see a normalized value across all systems. |  |
...
| Field  | Level  | Type  | Description  | Example  |
|---|---|---|---|---|
| <a name="url.href"></a>url.href | extended | keyword | Full url. The field is stored as keyword. | `https://elastic.co:443/search?q=elasticsearch#top` |
| <a name="url.scheme"></a>url.scheme | extended | keyword | Scheme of the request, such as "https".<br/>Note: The `:` is not part of the scheme. | `https` |
| <a name="url.hostname"></a>url.hostname | extended | keyword | Hostname of the request, such as "elastic.co".<br/>In some cases a URL may refer to an IP and/or port directly, without a domain name. In this case, the IP address would go to the `hostname` field. | `elastic.co` |
| <a name="url.port"></a>url.port | extended | integer | Port of the request, such as 443. | `443` |
| <a name="url.path"></a>url.path | extended | keyword | Path of the request, such as "/search". |  |
| <a name="url.query"></a>url.query | extended | keyword | The query field describes the query string of the request, such as "q=elasticsearch".<br/>The `?` is excluded from the query string. If a URL contains no `?`, there is no query field. If there is a `?` but no query, the query field exists with an empty string. The `exists` query can be used to differentiate between the two cases. |  |
| <a name="url.fragment"></a>url.fragment | extended | keyword | Portion of the url after the `#`, such as "top".<br/>The `#` is not part of the fragment. |  |
| <a name="url.username"></a>url.username | extended | keyword | Username of the request. |  |
| <a name="url.password"></a>url.password | extended | keyword | Password of the request. |  |
...
| Field  | Level  | Type  | Description  | Example  |
|---|---|---|---|---|
| <a name="user.id"></a>user.id | core | keyword | One or multiple unique identifiers of the user. |  |
| <a name="user.name"></a>user.name | core | keyword | Name of the user.<br/>The field is a keyword, and will not be tokenized. |  |
| <a name="user.email"></a>user.email | extended | keyword | User email address. |  |
| <a name="user.hash"></a>user.hash | extended | keyword | Unique user hash to correlate information for a user in anonymized form.<br/>Useful if `user.id` or `user.name` contain confidential information and cannot be used. |  |
...
| Field  | Level  | Type  | Description  | Example  |
|---|---|---|---|---|
| <a name="user_agent.original"></a>user_agent.original | extended | keyword | Unparsed version of the user_agent. |  |
| <a name="user_agent.device"></a>user_agent.device | extended | keyword | Name of the physical device. |  |
| <a name="user_agent.version"></a>user_agent.version | extended | keyword | Version of the physical device. |  |
| <a name="user_agent.major"></a>user_agent.major | extended | long | Major version of the user agent. |  |
| <a name="user_agent.minor"></a>user_agent.minor | extended | long | Minor version of the user agent. |  |
| <a name="user_agent.patch"></a>user_agent.patch | extended | keyword | Patch version of the user agent. |  |
| <a name="user_agent.name"></a>user_agent.name | extended | keyword | Name of the user agent. | `Chrome` |
| <a name="user_agent.os.name"></a>user_agent.os.name | extended | keyword | Name of the operating system. |  |
| <a name="user_agent.os.version"></a>user_agent.os.version | extended | keyword | Version of the operating system. |  |
